        ...
        <div class="admin-panel" id="adminPanel">
            ...
            <div id="adminControls" style="display: none;">
                ...
                <div class="batch-update">
                    <h4>📊 Batch Update All Teams</h4>
                    <div class="team-inputs">
                        <div class="team-input-row">
                            <label>Team A Points:</label>
                            <input type="number" id="teamA" placeholder="0" min="0">
                        </div>
                        <div class="team-input-row">
                            <label>Team B Points:</label>
                            <input type="number" id="teamB" placeholder="0" min="0">
                        </div>
                        <div class="team-input-row">
                            <label>Team C Points:</label>
                            <input type="number" id="teamC" placeholder="0" min="0">
                        </div>
                        <div class="team-input-row">
                            <label>Team D Points:</label>
                            <input type="number" id="teamD" placeholder="0" min="0">
                        </div>
                    </div>
                    <div class="batch-buttons">
                        <button class="btn btn-primary" onclick="publishResults()">📢 Publish All Results</button>
                        <button class="btn btn-secondary" onclick="loadCurrentScores()">📝 Load Current Scores</button>
                    </div>
                </div>
                <hr style="margin: 20px 0; border: 1px solid rgba(255,255,255,0.2);">
                <div class="individual-update">
                    <h4>⚡ Quick Individual Update</h4>
    ...
    <script>
        ...
        function checkPassword() {
            ...
            if (password === ADMIN_PASSWORD) {
                ...
                alert('Incorrect password! Contact the admin.');
            }
        }

        function publishResults() {
            const teamAPoints = parseInt(document.getElementById('teamA').value) || 0;
            const teamBPoints = parseInt(document.getElementById('teamB').value) || 0;
            const teamCPoints = parseInt(document.getElementById('teamC').value) || 0;
            const teamDPoints = parseInt(document.getElementById('teamD').value) || 0;

            // Update all teams at once
            teams[0].points = teamAPoints; // Team A
            teams[1].points = teamBPoints; // Team B
            teams[2].points = teamCPoints; // Team C
            teams[3].points = teamDPoints; // Team D
            ...
            document.getElementById('teamC').value = '';
            document.getElementById('teamD').value = '';

            alert('✅ All results published successfully!');
        }

        function loadCurrentScores() {
            document.getElementById('teamA').value = teams.find(t => t.name === 'Team A').points;
            document.getElementById('teamB').value = teams.find(t => t.name === 'Team B').points;
            document.getElementById('teamC').value = teams.find(t => t.name === 'Team C').points;
            document.getElementById('teamD').value = teams.find(t => t.name === 'Team D').points;
        }
